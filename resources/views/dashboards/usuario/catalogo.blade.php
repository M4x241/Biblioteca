...
<head>
    ...
    <title>Biblioteca - Mi Catálogo</title>
...
<body class="bg-gray-100 min-h-screen flex text-gray-900 font-sans" x-data="{ modalOpen: false, selectedBook: {} }">
    ...
    @include('components.sidebar-logo')
        ...
        <div class="p-4 border-b">
            <form method="GET" action="{{ route('usuario.catalogo') }}" class="space-y-2">
        ...
        <nav class="flex-1 p-4 space-y-2 text-sm">
            ...
            <a href="{{ route('usuario.catalogo') }}"
                class="block px-3 py-2 rounded-md hover:bg-gray-200 {{ request()->routeIs('usuario.catalogo') ? 'font-semibold text-green-600 bg-green-50' : '' }}">Catálogo</a>

            <a href="{{ route('usuario.mis-prestamos') }}"
                class="block px-3 py-2 rounded-md hover:bg-gray-200 {{ request()->routeIs('usuario.mis-prestamos') ? 'font-semibold text-green-600 bg-green-50' : '' }}">Mis
                Préstamos</a>

            <a href="{{ route('usuario.mis-reservas') }}"
                class="block px-3 py-2 rounded-md hover:bg-gray-200 {{ request()->routeIs('usuario.mis-reservas') ? 'font-semibold text-green-600 bg-green-50' : '' }}">Mis
                Reservas</a>

            <a href="{{ route('usuario.perfil') }}"
                class="block px-3 py-2 rounded-md hover:bg-gray-200 {{ request()->routeIs('usuario.perfil') ? 'font-semibold text-green-600 bg-green-50' : '' }}">Mi
                Perfil</a>
        </nav>
  ...
  <!-- Contenido principal -->
  <main class="flex-1 flex flex-col items-center p-12 h-screen overflow-y-auto">
    <h1 class="text-3xl font-bold mb-4 text-center text-gray-800">Catálogo de Libros</h1>
<br>
<br>
        ...
        @if (session('success'))
            <div
                class="w-full max-w-5xl mb-6 px-4 py-3 bg-green-100 border border-green-400 text-green-700 rounded-lg flex items-center gap-2">
        ...
        @if (session('error'))
            <div
                class="w-full max-w-5xl mb-6 px-4 py-3 bg-red-100 border border-red-400 text-red-700 rounded-lg flex items-center gap-2">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
        ...
        @if ($errors->any())
            <div class="w-full max-w-5xl mb-6 px-4 py-3 bg-red-100 border border-red-400 text-red-700 rounded-lg">
        ...
        <!-- GRID DE LIBROS 4x2 (2 filas × 4 columnas = 8 libros) -->
        <div class="w-full max-w-5xl px-8">
            <div class="grid grid-cols-4 gap-8">
                ...
                @empty
                    ...
                    c-1.746 0-3.332.477-4.5 1.253" />
                            ...
                            <p class="font-medium">No hay libros disponibles en el catálogo</p>
                        </div>
                    </div>
                @endforelse
            </div>
            ...
            @if (isset($totalPages) && $totalPages > 1)
                <div class="mt-8 flex items-center justify-center space-x-4">
                    ...
                    <!-- Botón Anterior -->
                    @if ($hasPrev)
                        <a href="{{ route('usuario.catalogo', array_merge(['page' => $page - 1], isset($q) && $q !== '' ? ['q' => $q] : [])) }}"
        ...
        <div @click.stop class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 overflow-hidden">
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex gap-3 justify-end">
                ...
                <template x-if="!selectedBook.ocupado && !selectedBook.reservado">
                    @auth
                        <form :action="`{{ route('reservas.store') }}`" method="POST" class="inline">
                            ...
                            <input type="hidden" name="libro_id" :value="selectedBook.id">
                            <button type="submit"
                ...
                <template x-if="selectedBook.ocupado">
                    <span class="px-4 py-2 text-sm font-medium text-red-700 bg-red-100 rounded-md">
                        Ocupado
                    </span>
                </template>
                <template x-if="selectedBook.reservado">
                    <span class="px-4 py-2 text-sm font-medium text-yellow-700 bg-yellow-100 rounded-md">
                        Reservado
                    </span>
                </template>
            </div>
