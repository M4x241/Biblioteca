...
<body class="bg-gray-100 min-h-screen flex text-gray-900 font-sans">
    ...
    <aside class="w-64 bg-white border-r flex flex-col h-screen overflow-y-auto">
        ...
        <div class="p-4 border-b">
            <form method="GET" action="{{ route('dashboard.todos-libros') }}" class="space-y-2">
    ...
    <main class="flex-1 flex flex-col p-8 h-screen overflow-y-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-gray-800">Todos los Libros</h2>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($libros as $libro)
                            <tr class="hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <img class="h-10 w-10  object-cover"
                                                src="{{ asset($libro->imagen ?? 'images/libro-default.png') }}"
                                                alt="{{ $libro->titulo }}">
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-purple-600 text-white">
                                        {{ $libro->categoria ?? 'Sin categoría' }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-500 max-w-xs">
                                    <div class="truncate" title="{{ $libro->sinopsis }}">
        ...
        @if (isset($totalPages) && $totalPages > 1)
            <div class="mt-6 flex items-center justify-center space-x-4">
                @if ($hasPrev)
                    <a href="{{ route('dashboard.todos-libros', ['page' => $page - 1, 'q' => $q, 'genero' => $genero]) }}"
